...
serve(async (req) => {
  ...
  try {
    ...
    const { email, password, fullName, phone, username } = await req.json();
    ...
    // Normalize username (e.g. "@ajay0002" -> "ajay0002@nationalacademy.internal")
    ...
    return new Response(JSON.stringify({ success: true, adminId: uid }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
